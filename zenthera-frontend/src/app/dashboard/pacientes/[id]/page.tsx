...
import { AlertCircle, ArrowLeft, Loader2, Save, UserSquare2, RefreshCcw } from 'lucide-react';
...
export default function DetallePacientePage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const mutation = useMutation({
    ...
    onError: (error: any) => {
      setServerError(error.response?.data?.message || 'Error al actualizar el paciente');
    }
  });
        ...
        <div className="flex flex-col items-center justify-center min-h-[60vh] max-w-md mx-auto text-center">
          ...
          <p className="text-foreground/70 mb-8">
            {isNotFound
              ...
              : ((error as any)?.response?.data?.message || 'Ocurrió un error inesperado al cargar los datos.')}
          </p>
        ...
        <header className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-8">
          <div>
            ...
            <p className="text-foreground/60 text-sm">
              Actualiza la información personal y médica de {data.data.nombres}.
            </p>
          </div>
        </header>
